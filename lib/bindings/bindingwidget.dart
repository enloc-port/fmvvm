...
class BindingWidget<T extends BindableBase> extends StatefulWidget {
  ...
  Object getValue(String key, {Object? converterParameter}) {
    ...
    if (binding.bindingDirection == BindingDirection.OneTime && !(binding.originalValue is _OriginalValueNeverSet)) {
      returnValue = binding.originalValue;
    } else if (binding.valueConverter == null || converterParameter == null) {
      returnValue = binding.source.getValue(binding.sourceProperty);
    } else {
      returnValue = binding.valueConverter!.convert(binding.source, binding.source.getValue(binding.sourceProperty), parameter: converterParameter);
    }

    if (binding.originalValue is _OriginalValueNeverSet && binding.bindingDirection == BindingDirection.OneTime) {
  ...
  void setValue(String key, Object value, {Object? converterParameter}) {
    Binding binding = _getBinding(key);
    if (binding.valueConverter == null || converterParameter == null) {
      binding.source.setValue(binding.sourceProperty, value);
    } else {
      binding.source.setValue(binding.sourceProperty, binding.valueConverter!.convertBack(binding.source, value, parameter: converterParameter));
    }
  }
...
class _BindingWidgetState<T> extends State<BindingWidget> {
  ...
  void _createListener(Binding binding, List<BindableBase> addedListeners) {
    if (!addedListeners.any((b) => b == binding.source)) {
      var propertyListener = (String fieldName) {
        if (binding.bindingDirection == BindingDirection.TwoWay && (fieldName == "" || _sourceBindings.any((b) => b.sourceProperty.name == fieldName && b.source == binding.source))) {
          setState(() {});
        }
      };
      ...
      if (binding.source.getValue(binding.sourceProperty) is NotificationList) {
        var notificationList = binding.source.getValue(binding.sourceProperty) as NotificationList;
        var listListener = () {
          setState(() {});
        };
  ...
  void dispose() {
    _bindingListeners.forEach((bl) {
      var binding = _sourceBindings.singleWhere((sb) => sb.key == bl.bindingKey);
      if (bl.isListListener) {
        NotificationList<dynamic> notificationList = binding.source.getValue(binding.sourceProperty) as NotificationList;
        notificationList.removeListener(bl.listener as VoidCallback);
      } else {
        binding.source.removePropertyListener(bl.listener as PropertyChangedListener);
      }
    });
    super.dispose();
  }
...
class _BindingListener {
  const _BindingListener(this.bindingKey, this.listener, this.isListListener);

  final bool isListListener;
